...
from .forms import NewCommentForm, NewCollabForm
...
class net(LoginRequiredMixin, ListView):
    model = Post
    template_name = 'blog/net.html'
    context_object_name = 'posts'
    ordering = ['-date_posted']
    paginate_by = PAGINATION_COUNT
...
class UserPostListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Comment.objects.filter(post_connected=user)
...
class PostcollabView(DetailView):
    model = Post
    template_name = 'blog/user_posts.html'
    context_object_name = 'post'
    ...
    def get_context_data(self, **kwargs):
        data = super().get_context_data(**kwargs)
        collabs_connected = Collab.objects.filter(post_connected=self.get_object()).order_by('-date_posted')
        data['collabs'] = collabs_connected
        data['form'] = NewCollabForm(instance=self.request.user)
        return data

    def post(self, request, *args, **kwargs):
        new_collab = Collab(content=request.POST.get('content'),
                              author=self.request.user,
                              post_connected=self.get_object())
        new_collab.save()
